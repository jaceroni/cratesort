...
### Classify Library button — hidden, not disabled, when nothing's left (updated August 2026)

`_refresh_classify_btn()` is called at the end of every `load()`. It **hides** the Classify Library button (`setVisible`, not `setEnabled`) when `_count_unclassified_artists()` returns 0 — same "hide a dead-end action rather than show it muted" convention used for `_consolidate_btn` on the duplicate-review screen. `_count_unclassified_artists()` only counts artists whose genre is in the Unclassified set AND who have no explicit `genre` entry in `self._edits` (i.e., `library_edits.json`). Artists explicitly set to Unclassified via right-click or via `_exit_classify_mode_accept` are counted as handled. If new tracks are added and rescanned, their artists have no edits entry → count > 0 → button reappears automatically.

**A "Reclassify Library" active button (replacing the muted state) was built, tested, and then explicitly reverted the same day — do not re-add it without re-reading this.** The idea: once nothing's unclassified, turn the dead button into an active one that forces a fresh classification pass. Rejected after direct discussion because it provides no real value: the dashboard already reruns classification automatically on *every single app launch* (`_start_classification_phase`, not just first-run), so by the time Library is visible the session is already as fresh as this launch can make it. The only hypothetical extra value — catching a manual "Change Genre…" mis-edit — doesn't hold up either: "Change Genre…" never writes to the audio file (only `library_edits.json`), so the classifier's own read of a track never changes just because a user overrode its genre; the persistent Status column (`✓ Approved` / `✎ Edited` / `△ Unclassified`) already surfaces that exact discrepancy live, with no button click required. Net: no scenario exists where a manual reclassify reveals anything the automatic per-launch pass and the persistent Status column don't already show. Hiding the button (this section) is the final state.
...
## Locked decision — August 2026 (persistent classification Status column)
...
- **Fix:** `LC_CLS_CONF` (the "Confidence" column, index 13, `library_browser.py`) is now a dual-purpose, always-visible column once any classification data exists. During active classify-mode review it still shows Confidence (MATCHED/HIGH/MEDIUM/LOW/NONE), unchanged. Outside classify mode it relabels to **Status** and shows a persistent per-artist verdict: **✓ Approved** (green `#6B9E78`), **✎ Edited** (teal `#428175`), or **△ Unclassified** (red `#C75B5B`). Header text swaps dynamically via `headerItem().setText()`; visibility is controlled by `self._has_classification`, set every time `load()` runs, not by the classify-mode enter/exit toggle anymore.
- **Deliberately does NOT use `ArtistEntry.state`** (which already has matching labels built — `STATE_LABELS`/`STATE_COLORS` in `classifier_view.py:112-124` — but is unreliable in the live app: the `'approved'` transition only exists in `_ClassifierViewLegacy`, a class never instantiated anywhere; "Accept Reclassifications" never re-saves `entry.state`; "Reassign Artist" silently fails to persist its own state change). Building on that field as-is would show "Pending" forever for artists already reviewed and accepted.
- **Instead, `_derive_persistent_status(artist, current_genre)` (new, `library_browser.py`) compares the artist's final genre against the classifier's originally-proposed genre** — both already loaded by `load()` (`self._edits`/resolved genre, and `self._session_genre[artist]`), no new fragile state-tracking. Approved = final matches proposed (covers both "classifier nailed it, untouched" and "classifier proposed it, bulk-accepted"). Edited = final differs from proposed (or there was no proposal — pure manual assignment). Unclassified = final genre is still blank/Unclassified. Verified against the real Accept flow (not just unit logic): a manually-corrected artist shows Edited, an artist the acknowledgment pass swept in shows Unclassified, everything else shows Approved.
- **`LC_CLS_STATUS` (index 14, the old separate "Status" column) is now fully unused** — deliberately left defined rather than removed. Dropping a column changes `len(HEADERS)`/section count, and `QHeaderView.restoreState()` (used for both the QSettings cross-session column layout and the classify-mode enter/exit snapshot) silently fails and resets a user's saved column layout when the saved blob's section count doesn't match current. Keeping the inert column avoids that regression for the cost of one unused constant.
- **`restoreState()` gotcha, and the actual fix for it:** both `saveState()`/`restoreState()` call sites (QSettings init-time restore, and `_exit_classify_mode_cancel`'s pre-classify snapshot restore) bundle every column's hidden-flag into one blob — a blob saved before this feature existed has `LC_CLS_CONF` baked in as hidden. Verified empirically: `load()` unconditionally re-asserts `setColumnHidden(LC_CLS_CONF, not self._has_classification)` every time it runs, which force-corrects a stale hidden flag from any old saved blob. Confirmed via a real restoreState() round-trip in a test, not just read by inspection.

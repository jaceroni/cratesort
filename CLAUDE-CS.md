...
**Same-day follow-up, still 0.1.10-beta (app code unchanged, DMG re-packaged):** the DMG itself had no branding — see "What 'generic-looking DMG' was actually two bugs" below in the DMG build section. Fixed with a real branded background image + arranged icon layout, and the volume icon actually using the rendered/masked app icon instead of a flat raw asset. The `0.1.10-beta.dmg` filename didn't need to change since the app binary inside it is identical to the first 0.1.10 build — only the outer DMG packaging did.
...
**DMG build — now scripted (`packaging/build_dmg.sh`), fixed 2026-09-16.** Every build from 0.1.0-beta through 0.1.10-beta ran this pipeline as one-off hand-typed shell commands each time — that's how the volume-icon bug below survived ten releases unnoticed. Usage:
```bash
.build-venv/bin/pyinstaller packaging/CrateSort.spec --noconfirm --clean
packaging/build_dmg.sh
```
(regenerate `packaging/dmg_background.png` first with `.build-venv/bin/python packaging/generate_dmg_background.py` if brand assets or the DMG layout changed). The script reads the version from `cratesort/src/version.py` itself, so the output filename can't drift either.

**What "generic-looking DMG" was actually two bugs, both fixed 2026-09-16 (Jace: "looks like a nondescript, bullshit-ass, generic DMG"):**
1. **The mounted volume's icon was never fixed after all** — the "Not yet verified" flag from 2026-08-04 sat open for ten releases. Same root cause as the DMG file's own icon (below): the raw `cratesort/assets/icons/app/CrateSort.icns` asset is a flat, sharp-cornered square with no rounding baked in — macOS only auto-composites the polished rounded/shadowed look onto real `.app` BUNDLE icons, nothing else. The volume-icon step was still just `cp`-ing that raw flat asset straight onto `.VolumeIcon.icns`, so the mounted disk's icon looked visibly cruder/flatter than the real app icon sitting right next to it. **Fix:** reuse the exact same rendered/masked bitmap already being extracted off `dist/CrateSort.app` for the DMG file's own icon (see below) for the volume icon too — one extraction, two uses.
2. **There was no branded DMG window at all** — no background image, no arranged icon layout, just the Finder default: a plain gray window with `CrateSort.app`/`Applications`/`Uninstall CrateSort.app` sitting in a default icon grid. This is what most people actually picture when they say a DMG "looks generic" (compare to any commercial Mac app's branded "drag to Applications" installer screen). **Fix:** `packaging/generate_dmg_background.py` renders a 660×420 (@2x, 1320×840px) PNG via `QSvgRenderer`+`QPainter` — `#1a1a1a` background (matches the app/Dock-icon backdrop), the `cs-logo-lockup-horiz.svg` wordmark (already cream-`#f8efdd`, designed to sit on dark), the tagline, and a hand-drawn arrow connecting where the app icon and the Applications alias will sit. `build_dmg.sh` copies it into a hidden `.background/` folder inside the staging dir (so it ships inside the volume itself) and drives a Finder AppleScript pass on the mounted RW volume that sets window bounds, icon size (128px, unarranged), the background picture, and each item's exact position — `CrateSort.app` left, `Applications` alias right (the arrow connects them), `Uninstall CrateSort.app` in its own row below as a secondary item (no caption drawn near it in the background image — Finder's own filename label sits close enough beneath it that painted text there would collide with that native label).

**Real ordering bug hit while building this, worth knowing if this script is ever restructured:** the volume-icon block (`cp .VolumeIcon.icns` + `SetFile -c icnC` + `SetFile -a C`) must run **after** the Finder AppleScript layout pass, not before. Setting it first and then running the `tell application "Finder" … set background picture / set position of item …` pass silently wiped both the icon file and the volume's custom-icon flag by the time the disk was detached — confirmed by inspecting the mounted volume afterward (`.VolumeIcon.icns` was gone, `GetFileInfo` showed the custom-icon flag unset). Root cause is presumably Finder's own window/icon-view-options write touching the same `com.apple.FinderInfo` structure that carries the custom-icon bit and clobbering it. `build_dmg.sh` now does the Finder layout pass first, the volume icon last, with nothing but `sync`+`hdiutil detach` after it.

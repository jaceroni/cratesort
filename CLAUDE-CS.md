...
### "NO DRIVE NEEDED" tools section (added 2026-09-16)

Jace: no drive connected meant no way to reach the YouTube-import / local-conversion tools at all — they only ever lived inside `_build_action_cards_section()`, which is part of the dashboard (stack index 1), unreachable while stack index 0 (welcome) is showing. Neither tool actually needs a library — `_open_yt_import`/`_open_convert` already degrade gracefully with `self._library_path is None` (`_YTImportDialog` falls back its destination to `library_path or Path.home() / 'Downloads'`; `_ConvertDialog` never touches library data at all) — this was a pure navigation gap, not a tool limitation.

Fix: extracted the YouTube+conversion card grids out of `_build_action_cards_section()` into a shared `_build_yt_convert_cards_section() -> QWidget` (goto cards / the divider above them stay in `_build_action_cards_section`, which now just calls the shared method). `_build_welcome()` renders that same section below `welcome_card`, unconditionally across all three welcome branches (no saved path / saved path not found / saved path found) — a "NO DRIVE NEEDED" eyebrow + one-line note, then the section fixed at 620px width (2 cards per row, centred via the same stretch-`QHBoxLayout` pattern as `welcome_card`, not `addWidget(..., alignment=…)` — see the height-for-width note above). So the tools are now reachable at every point before a library is picked, not just after a scan starts.
...
**Drag-to-resize grip (2026-09-07):** frameless `Tool` windows have no OS edge to grab, so every dialog gets a self-painted `_CornerResizeGrip(QSizeGrip)` in the bottom-right corner (six muted-cream dots, `Drag to resize` tooltip; plain `QSizeGrip` paints nothing visible on macOS). `_CrateSortDialog.__init__` creates `self._size_grip` and `self._resizable = True`. `showEvent`: after `adjustSize()`, if the natural size exceeds the parent window it's clamped to `parent − 80px` (a content-sized dialog — long change description, deep path — could otherwise render wider than the app); then, unless the subclass pinned BOTH dimensions (`setFixedWidth` **and** `setFixedHeight` → `minimumWidth==maximumWidth and minimumHeight==maximumHeight`, detected as `_locked` → grip hidden), the grip is shown and `setMinimumHeight(natural)` becomes the shrink floor. `resizeEvent` repositions + raises the grip every frame (cheap; also runs through the bounce animation). QSizeGrip honours the dialog's `minimum*/maximum*`, so a drag can never go below the designed floor. `done()`'s existing `setMinimumSize(0,0)` still lets the exit animation shrink past it. A subclass that must stay fixed can set `self._resizable = False` after `super().__init__()`.

**`_fit_dialog_width(dialog, samples, *, chrome, minimum, maximum)` — `overlays.py` (2026-09-07):** raises `dialog.minimumWidth()` so the widest string in `samples` (measured with the dialog's `QFontMetrics`) fits on one line plus `chrome` px of row furniture (icons, timestamps, buttons, margins), clamped to `[minimum, maximum]`. Used by dialogs whose rows hold variable-length content that the old fixed width sliced off — `_ChangeReviewDialog` (change descriptions), `_GatherStragglersDialog` (source-folder paths), `_UnreadableFilesDialog` (filenames + parent folders). Past `maximum` the row text is expected to elide with a tooltip; the resize grip then goes wider.

**Usage:** subclass `_CrateSortDialog`, add a `QFrame` container with dark-panel styling, lay out content inside it. The overlay, animation, and resize grip are handled automatically.
...
2. **Action Cards** (`_build_action_cards_section(scanning=False)`) — three groups, separated by dividers (16px clear space each side, matching the divider above this whole section — `vbox.addSpacing(6)` on top of the inner layout's uniform 10px spacing):
   ...
   - These two rows are built by the shared `_build_yt_convert_cards_section()` (extracted 2026-09-16), also rendered on the welcome screen so they're reachable with no drive connected — see "NO DRIVE NEEDED" tools section under Launch Screen Architecture above.
...
`_ChangeReviewDialog` groups the change list **under shared timestamp headers**: the grouping loop in `__init__` walks `self._changes` in order and emits a `_build_time_header('Today at 12:34 AM', first=…)` label whenever the formatted `_fmt_time(mtime)` differs from the previous row's (a single Serato session usually stamps everything the same minute, so this collapses a column of identical timestamps into one header). Changes with no `mtime` get no header. Each change **row** (`_build_row`, one `QHBoxLayout` per `QFrame`) is then just `[dot · description(_ElidingLabel, stretch) · keep radio · remove radio]` — no per-row timestamp. Design history: originally one line (description + timestamp + both radios) which `adjustSize()` stretched to ~830px wide → tried radios on their own indented second line (modal 540 but felt cramped/empty) → per-row timestamp eyebrow (2026-09-10) → **grouped headers** (2026-09-10, current): modal ~640px, no repeated timestamps. The radios are a **two-option `QRadioButton` pair** (exclusive `QButtonGroup` per row, first option checked by default) — **not** push buttons, and **not** generic "Keep"/"Undo" labels. The label text is looked up per change-type from `_RADIO_LABELS` and states the concrete resulting outcome for the crate/tracks themselves:
...
Dialog sizing: `setMinimumWidth(540)` floor, then **`_fit_dialog_width(self, [c['description'] …], chrome=360, minimum=560, maximum=900)`** widens it to fit the longest change description on one line. `chrome` covers the dot + both radios + dialog margins (the timestamp is on its own eyebrow line, not in the main row). Screenshot-typical content lands ~640px wide. No forced minimum height (was `setMinimumSize(540, 480)`, which left a large dead gap below a single-row change list since the row-scroll area had `stretch=1` and was forced to fill the fixed 480px floor). The per-row description is an **`_ElidingLabel`** (`elide=ElideMiddle`, `setFixedHeight(18)` to override its default 32) with the full text as a tooltip — it elides rather than hard-clips when the dialog is dragged narrower or the line exceeds 900. The row-list `QScrollArea` has `setMaximumHeight(440)` (scrolls past ~8 rows + headers) and `setHorizontalScrollBarPolicy(ScrollBarAlwaysOff)` (a horizontal scrollbar was transiently appearing on row-state changes — layout-recompute artifact, not real overflow). The dialog also carries the shared corner resize grip (see `_CrateSortDialog` above).
...
**Metadata-field text clipping, fixed 2026-09-16 (real bug, Jace confirmed live on screen, not a photo artifact):** the URL input and every metadata field (`_field()`, used for Artist/Title/Album/Year/Genre) share `_FIELD_INPUT_STYLE` (13px font, `padding: 8px 10px`) with no explicit height — its own `sizeHint()` correctly computes 36px (verified identical to `_CONTROL_HEIGHT` used elsewhere in the app), but the fields sit in `meta_grid` (a `QGridLayout` nested inside `meta_frame`, itself inside the dialog's outer `QVBoxLayout`), and the row actually allocated only 26px — a real, reproducible 10px shortfall confirmed by direct geometry measurement (`.geometry()` vs `.sizeHint()`), independent of window size or the global theme stylesheet. This is the same class of bug as `[[project_pyqt_gotchas]]` #15 ("a QVBoxLayout's cached `sizeHint()` can go permanently stale once a dialog is first shown, and neither `.invalidate()` nor `.activate()` reliably un-sticks it") one level down, in a nested `QGridLayout` — confirmed via the same reproduction method (`.invalidate()`+`.activate()` on `meta_frame`'s layout did not fix it). **Fix, matching this file's own established fix pattern for the same gotcha** ("stop asking Qt for the number... compute the target size directly"): both `_url_input` and `_field()` now call `setFixedHeight(36)` — a hard widget constraint the layout can't silently shrink, unlike a merely-requested `sizeHint()`. Fixing the fields alone surfaced the identical bug one level up (the GENRE row spilling ~9px past `meta_frame`'s own allocated bottom edge, which would have overlapped the Artwork section below it) — closed by also flooring `meta_frame.setMinimumHeight(meta_grid.sizeHint().height())`, read fresh at construction time right after all 8 grid rows are populated. Verified across parent-window heights from 700px to 2000px: every field renders at its correct 36px, `meta_frame` fully contains all five rows with no overflow, and the Import/Cancel buttons still fit with margin below.
...
**Real-world 75k-track-library findings, fixed 2026-09-18.** Jace field-tested on a DJ friend's real library while traveling (75,000 files, mostly on an HDD) — three freezes reported ("spinning wheel of death for like a good minute," almost force-quit) plus one layout bug, all real, all root-caused by reading the actual code rather than assumed to be "just the HDD":

1. **End of scan → dashboard appearing.** `DuplicateDetector.detect()` (`core/duplicate_detector.py`) already runs off the main thread via `dashboard.py`'s `_BgSteps`, and its bucket-then-cluster algorithm isn't the O(n²) it might look like at first glance — but a tight pure-Python loop over tens of thousands of buckets, with zero yield points, still starves the main thread's GIL time badly enough that the whole app reads as frozen (a real OS-level beachball, not just a slow repaint) even though nothing is technically deadlocked. **Fix:** `time.sleep(0)` every 200 buckets in `_fast_pass()`'s main loop forces a GIL handoff so Qt's event loop actually gets scheduled in between.
2. **Opening "Review Duplicates" at 16,000 groups.** `_populate_results()` built a real Qt widget — collapsed strip, or the accepted/dismissed/expanded variant — for **every single group**, synchronously, before the screen could show at all. The existing "cheap collapsed strip" design (above) was scoped against ~700 duplicates during the original Rinse rework; at 16,000 the *cheap* strip's construction cost alone adds up to the reported freeze. **Fix:** `_build_populate_plan()` now separates *what* to show (headers, empty states — cheap, computed immediately) from *building the group-card widgets themselves* (deferred). Below `_CHUNK_THRESHOLD` (800 groups) it still runs in one synchronous pass — fast enough that a progress screen would just flash. Above it, `_continue_populate()` builds `_CHUNK_SIZE` (200) cards per event-loop tick via `QTimer.singleShot(0, …)`, entirely on the **progress stack page** (state 1, reusing the existing consolidation-progress widgets, not a new screen) so nothing half-renders in front of the user, with a real determinate progress bar (the total group count is always known up front — never a fake/pulsing indicator here). `_populate_gen` is bumped on every `_populate_results()`/`hideEvent()` call so a stale in-flight chunked run (a rapid filter click, or navigating away mid-build) recognizes itself as superseded and drops instead of racing a newer one; `_populating_in_progress` distinguishes "progress screen showing because we're still building the list" from "showing because a real consolidation is running" (same stack page, different reason — `hideEvent` must reset the stack for the former but leave the latter alone).
3. **Clicking "Accept all remaining true duplicates."** `_on_accept_all_true()` looped every accepted index and called `_apply_card_change()` (already the efficient one-card-in-place rebuild, not a full re-populate) synchronously for each — fine per-item, not fine at thousands of items. **Fix:** identical chunked-with-progress treatment via `_continue_accept_all()`, sharing `_populate_gen`/`_CHUNK_THRESHOLD`/`_CHUNK_SIZE` with the populate path above.
4. **Expanded group card overflowing wider than the window, hiding the per-group Accept button off-screen to the right — "no matter how big we made the app."** `_build_copy_row()`'s location label (`LOCATION: <folder path>`) and `_build_group_card()`/`_build_collapsed_card()`'s artist–title label had no `setWordWrap`, so a genuinely deep real-library folder path (or a long title) reported an unbounded `sizeHint()` width; since the results `QScrollArea` uses `setWidgetResizable(True)`, that dragged the content widget — and every card in it, footer buttons included — wider than the viewport, triggering a horizontal scrollbar that pushed the per-group Accept button out of view. **Fix:** `setWordWrap(True)` on all three labels (`path_lbl`, both `song_lbl`s, and `name_lbl` defensively for pathological filenames) — a wrapped label can never force its row wider than whatever width the layout actually gives it, at the cost of an occasional extra line on that one row. Verified with an artificially deep path + long title: no horizontal scrollbar, Accept button's right edge stays inside the viewport.

All four verified headless (chunking timing/state transitions, small-vs-large-library thresholds, hide-mid-chunk cleanup, and the overflow fix) before being called done — see `[[project_dashboard_scale_freezes]]`.
...
- A bare `"soundtrack"` genre/style tag does NOT route to Orchestral — too many OSTs are pop/rock song compilations, not composed scores. Only specific classical/score terms (`classical`, `symphony`, `film score`, `opera`, etc. — see `STYLE_MAP` in `classifier.py`) do. This is separate from the `Film & TV` content-type bucket, which is folder-triggered and unrelated to audio-only orchestral scores — a real soundtrack song or score sitting outside a "Film & TV" folder still classifies normally (Orchestral for scores, its actual genre for songs).
- Stand-up/spoken-word comedy bits (e.g. Eddie Murphy, Richard Pryor audio clips) are `Comedy`, the 16th genre (added 2026-09-18) — distinct from `Specialty`, which stays reserved for DJ-tool content (drops, FX, jingles). Trigger: ID3 genre tag `Comedy`/`Spoken Word`/`Stand-Up` (Tier 1 exact match or `STYLE_MAP`), or a folder named "Comedy"/"Stand-Up" as a Tier 5 fallback.
...
### Film & TV — a content-type bucket, not a genre (added 2026-09-04; audio included 2026-09-18)

`Film & TV` (`FILM_TV_GENRE` in `classifier_view.py`) is deliberately **not** in `PARENT_GENRES` — it's a content type, not a music genre, and doesn't count toward the 16 above or the "Why Only These Genres?" explainer. It still gets its own top-level `Media/Film & TV/<Show or Movie>/` folder and its own sidebar bucket in Library (the sidebar builds itself from whatever genre strings exist on entries, so no separate wiring was needed there).

**Trigger**: any file — video *or* audio, as of 2026-09-18 — whose ancestor folder is literally named "Film & TV" or "Film and TV" (case-insensitive) — checked against `rec.path.parts`, not a substring match. This is deliberately folder-based, not tag- or artist-based: a "Music Videos" folder gets zero special treatment and flows through normal per-artist genre classification exactly like audio. The `is_video` gate was dropped so short audio-only scene/dialogue clips (no accompanying video file) land in the same bucket as video clips of the same show/movie, rather than falling into Specialty; a real soundtrack song or score doesn't normally live inside a folder named "Film & TV" so this doesn't pull those in.

**Why this needed to be structural, not just a per-track tag**: the existing pipeline groups tracks by canonical artist-name string, then majority-votes ONE genre across the whole group (`classifier_view.py` `_ClassifyWorker`, and separately `library_browser.py` `_rebuild_tree` for the Library tab tree). A movie or TV show can share its title with a real recording artist — the flagship case being **Scarface** (the rapper) vs. *Scarface* (the movie) — and if the clip were only tagged `genre='Film & TV'` per-track while staying grouped under artist "Scarface", the group's majority vote (or the tab's ID3-vote fallback) would still overwrite it with whatever the rapper's audio tracks vote for. The fix pulls Film & TV clips out **before** artist grouping happens at all, in both places independently:
- `classifier_view.py` `_ClassifyWorker.run()`: folder-flagged clips (video or audio) are grouped by show/movie title (`_film_tv_title()` — prefers the artist/©ART tag, falls back to the leading `" - "`-delimited filename segment) into their own `ArtistEntry` objects, `is_film_tv=True`, genre forced to `Film & TV`, never entering the artist-name vote pool.
- `library_browser.py` `_rebuild_tree()`: same folder-based carve-out, independently, since this tree does its own artist-name grouping over `self._inventory` rather than consuming `ClassificationSession` results directly.
...
**Status: macOS beta packaging shipped — 0.1.0-beta (2026-07-12) through 0.2-beta (2026-09-19). Unsigned, no notarization.**

**0.2-beta (2026-09-19)** — first non-patch version bump (0.1.11 → 0.2), cut to mark the end of a long live-testing session against Jace's real ~20k-track library on `/Volumes/JWBC 2TB`. Everything in [[project_comedy_genre]], [[project_dedup_and_crate_sync_fixes]], and Round 10/11 of [[project_rinse_bulk_review]] is in this build:
- Comedy genre (16th) + Film & TV folder-trigger now includes audio, not just video.
- Two real dedup false-positive bugs fixed: instrumental/acapella-vs-vocal, and audio-vs-video (an MP4 was being recommended as the "winner" over its MP3 duplicates, which would have deleted the only audio copies).
- The Serato crate-sync scare from this session — verified no data was actually lost, then fixed both real causes: an unfiltered `._` AppleDouble sidecar scan that misreported endless "new crates," and a checkpoint that re-derived state via a separate rescan instead of the consolidation's own write (causing inflated, wrong "tracks removed" counts). Added a real "Undo This Consolidation" action — didn't exist before.
- Rinse screen renamed "Duplicate Consolidation" (was "Rinse Your Library" — Jace: the word "Rinse" only ever appeared at the two bookends of the flow with nothing tying it together elsewhere, so dropped rather than reinforced); completion screen "Rinsed." → "Consolidation Successful". Filter pills simplified to content-type only (All/True Duplicates/Possible Variants — dropped Pending/Accepted as filter buttons, accepted groups just sort to the bottom now). Winner-reasoning and metadata-conflict text consolidated into one line (was two, could disagree with each other). Pre-consolidation confirm dialog and the post-consolidation success screen both rebuilt around `_AnimatedStatCardWidget` (the dashboard's own stat-card look) instead of a paragraph of prose.

**Version single-source-of-truth, fixed 2026-09-16 (real bug, caught by Jace reading the app's own title bar):** `packaging/CrateSort.spec`'s `info_plist` version fields (what macOS/the DMG filename see) and `main_window.py`'s in-app `VERSION` constant (window title + About dialog — what the *user* sees) were **two separate hardcoded strings**. Every release from 0.1.1-beta through 0.1.9-beta bumped only the spec file, so the app's own title bar and About box sat frozen at "0.1.0" for nine straight releases while the actual packaged version silently climbed — nobody had a reason to open the About dialog and notice. Fixed with a new **`cratesort/src/version.py`** (`VERSION = "0.1.10"`, one constant) — `main_window.py` imports it directly; `CrateSort.spec` reads the same file as a standalone text blob (`exec()`'d into a throwaway namespace, not `import cratesort...`, so packaging can never pull in the app's own PyQt6 imports as a side effect) and uses it for both `CFBundleShortVersionString`/`CFBundleVersion`. `cratesort/pyproject.toml`'s `version` field is bumped by hand alongside it for cosmetic consistency (nothing reads it at runtime, so it can't cause this class of bug, but keep it roughly in sync). **From 0.1.10-beta on, bump `cratesort/src/version.py` ONLY — everything else follows automatically.**

**Same-day follow-up, still 0.1.10-beta (app code unchanged, DMG re-packaged twice more):** see the DMG build section below for the full story — a branded background/layout was tried and reverted per Jace's direction, the volume icon fix was kept, and the DMG file's own icon still isn't confirmed visible to Jace despite verified-correct data. The `0.1.10-beta.dmg` filename never needed to change across any of this — the app binary inside is identical to the first 0.1.10 build, only the outer DMG packaging did.

**0.1.7 through 0.1.9-beta** were same-day iterative fixes on top of 0.1.6, each a single version bump + rebuild with no packaging-process changes: **0.1.7** — the dashboard's YouTube-import/local-conversion cards (`_build_yt_convert_cards_section()`) rendered on the welcome screen too, reachable with no drive/library connected (`[[project_yt_import]]`). **0.1.8** — yt-dlp's own internal postprocessors (not just CrateSort's direct ffmpeg calls) need `ffmpeg_location` explicitly set or they only search `$PATH`, which is bare in a packaged `.app`; hit as "ERROR: Postprocessing: ffprobe and ffmpeg not found" at ~87% during a real YouTube import. **0.1.9** — the Artist/Title/Album/Year/Genre metadata fields' own `sizeHint()` was correct (36px) but the nested `QGridLayout` they sit in was allocating them only 26px, clipping the tops of capital letters/digits — a `[[project_pyqt_gotchas]]` #26 case, fixed with `setFixedHeight(36)` plus flooring the container's minimum height. All three verified headless before being folded into their DMG.

**0.1.6-beta** bundles everything accumulated on `scan-organize-robustness` since 0.1.5: the scan/organize robustness pass (`[[project-scan-process-isolation]]` — killable worker-process pool for tag reads, 3-phase scanner, scan.log + unreadable-files banner/dialog, classification cache-skip, `detect_stragglers` 3.24s→0.21s, `_BgSteps` off-thread post-scan analysis, scanning-banner one-line layout, Organize Cancel + slow-file indicator), out-of-process playback (`[[project-out-of-process-playback]]`), the crate-table virtualization fix, Latin/Orchestral genres + Film & TV bucket, the classify-perf fix, and the dialog resize-grip / content-fit-width / `_ChangeReviewDialog` timestamp-grouping work (`[[project-resizable-dialogs]]`). Version bumped in `packaging/CrateSort.spec` `info_plist` only, same documented pipeline below — no packaging-process changes. Built + verified (app launches clean, DMG checksum passes, mounts with both `CrateSort.app` and `Uninstall CrateSort.app` present) but **not yet manually smoke-tested by Jace** — that real-drive Manage-Library + Organize pass is still worth doing before wider distribution.
...
**Second half of the same bug, found 2026-09-16 in the packaged 0.1.7-beta DMG**: bundling the binary into the `.app` isn't enough on its own — **yt-dlp's own internal postprocessors** (`FFmpegExtractAudio` for the MP3 path, its merge/remux step for the MP4 path) shell out to ffmpeg *themselves*, independent of anything `ffmpeg_tools.py` does, and by default only look on `$PATH`. A frozen `.app` launched from Finder has a minimal `$PATH` (no Homebrew, and the bundled `imageio_ffmpeg` binary isn't on `$PATH` either) — so a YouTube import got exactly to "CONVERTING… 87%" before yt-dlp raised `ERROR: Postprocessing: ffprobe and ffmpeg not found. Please install or provide the path using --ffmpeg-location`. `yt_import_dialog.py`'s `_run_mp4`/`_run_mp3` `base_opts` dicts didn't set yt-dlp's `ffmpeg_location` option — its own manual `subprocess.Popen([get_ffmpeg_path(), …])` re-encode call in `_run_mp4` was already correct, but that only covers the final CrateSort-side re-encode, not yt-dlp's own extraction/merge step. Fix: both `base_opts` dicts now set `'ffmpeg_location': get_ffmpeg_path()`. Verified with `PATH` stripped to `/usr/bin:/bin` (mimicking the frozen app) that yt-dlp's `FFmpegExtractAudioPP.available` is `True` and resolves `.executable` to the bundled binary — it doesn't hard-require `ffprobe` to be present alongside it, it only errors when it can find *neither*.
...
**DMG build — now scripted (`packaging/build_dmg.sh`), fixed 2026-09-16.** Every build from 0.1.0-beta through 0.1.10-beta ran this pipeline as one-off hand-typed shell commands each time. Usage:
```bash
.build-venv/bin/pyinstaller packaging/CrateSort.spec --noconfirm --clean
packaging/build_dmg.sh
```
The script reads the version from `cratesort/src/version.py` itself, so the output filename can't drift either. **Plain default Finder window — no background image, no custom icon layout.** A branded background image + arranged icon positions (`packaging/generate_dmg_background.py`, a Finder AppleScript layout pass in `build_dmg.sh`) was built 2026-09-16 and **explicitly reverted same-day per Jace's direction** — he only asked about the file's own icon, not a branded install window, and didn't like the unsolicited result ("ugly," "I didn't ask for that"). Don't reintroduce a background/layout without being asked; if asked again, the reverted commit history has the working implementation to resurrect rather than starting over.

**Volume icon fix (kept, 2026-09-16):** the "Not yet verified" flag from 2026-08-04 below turned out to still be broken — the mounted volume's icon (`.VolumeIcon.icns`) was a straight `cp` of the raw flat `CrateSort.icns` asset, never the rendered/masked bitmap the DMG file's own icon uses (same root cause as that fix, see below), so it looked visibly cruder than the real app icon next to it. Fixed by reusing the same rendered-icon extraction for both, now embedded in `build_dmg.sh`.

**Open, unresolved as of 2026-09-16: the DMG file's own custom icon (see the locked method below) is verified byte-correct on disk — `SetFile -a C` flag confirmed set (against a clean control-file test, not just assumed), a real 1MB+ icon bitmap attached via `com.apple.ResourceFork`, surviving `cp` to multiple locations (Desktop, `/tmp`) — but Jace reports it still shows generic in Finder after a `killall Finder`+`killall Dock` refresh, the exact fix that worked for this same class of bug through 0.1.4-beta (2026-08-04).** Not yet root-caused. Whole project tree (`_dev/`, including `dist_dmg/`) lives inside a Dropbox-synced folder — a live theory, not confirmed — but a same-machine `cp` to Desktop/`/tmp` preserved the data unchanged (expected, since `cp` copies xattrs regardless of destination; that test doesn't actually rule Dropbox in or out, since the failure mode being tested for is Dropbox's own upload/sync processing, not a local copy). No `sudo` available in the environment these builds run from, so the deep icon-services cache (`/Library/Caches/com.apple.iconservices.store`) has never been cleared as a test — that, or a full log-out/log-in or reboot, is the next thing to try and needs to happen on Jace's side. **Before touching this again**: confirm with Cmd+I (Get Info, which reads live rather than from Finder's icon cache) whether the correct icon shows there — that isolates a stale-cache rendering issue (fixable with a deeper cache clear/reboot) from a genuine data problem (which the verification above makes unlikely, but hasn't been fully excluded).
...
The raw `CrateSort.icns` asset is a flat, sharp-cornered square — it has no rounding baked into its pixels. The polished rounded-squircle-with-shadow look only exists because **macOS auto-composites that treatment onto real `.app` bundle icons specifically** (Dock/Finder behavior for genuine application bundles, on any Mac, every time — not something baked into the icon file). The legacy `sips -i` (self-icon) → `DeRez -only icns` → `Rez -append` trick used for 0.1.0-beta happened to carry over an already-masked bitmap; attempting the identical steps for 0.1.1-beta produced a flat, unmasked icon instead — that tool chain is unreliable and not the actual source of the correct look. **Locked approach** (now embedded in `build_dmg.sh`, not a manual step): build `dist/CrateSort.app` first, grab Finder's actual rendered (auto-masked) icon straight off that real bundle via `NSWorkspace.iconForFile()` (JXA), downscale into the standard iconset sizes (`sips -z`), pack with `iconutil -c icns`, apply via the modern `NSWorkspace.setIconForFileOptions` API (not the legacy resource-fork trick) — to both the DMG file itself and, as of 2026-09-16, the volume icon too. If Finder still shows an old icon after a rebuild, it's *supposed* to be a cache issue, not a data issue, fixed by `killall Finder` — but see the open item above, that didn't hold this time.

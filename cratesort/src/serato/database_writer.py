...
def update_comment(
    ...
    """
    Update (or insert) the `tcom` field for one track in Serato's `database V2`.

    Serato keeps its own copy of the comment (the `tcom` field), separate from
    the audio file's own tag — see database_reader.py's read_track_metadata
    docstring. Writing only to the file's tag (metadata_merger.py's
    _write_comment) does not update what Serato itself displays, so this is
    the counterpart that actually makes a merged comment visible in Serato,
    mirroring update_play_count() above.
    ...
    patched, new_data = _patch_tcom(data, target_keys, new_comment)
    if not patched:
        logger.warning(
            '[DatabaseWriter] Track not found in database V2: %s', file_path_posix
        )
    ...
    try:
        ...
        logger.info('[DatabaseWriter] Updated comment for %s', file_path_posix)
        return True
    except OSError as exc:
        logger.warning('[DatabaseWriter] Write failed for %s: %s', db_path, exc)
        return False
...
def _patch_tcom(
    ...
    """
    Scan `data` for the `otrk` record matching any key in `target_keys`.
    Within that record, update or insert a `tcom` field with `new_comment`.
    Returns (patched: bool, new_data: bytes).
    """
    out    = bytearray()
    pos    = 0
    ...
    while pos + 8 <= length:
        ...
        if tag_str == 'otrk' and not found:
            file_path = _extract_pfil(record_data)
            if file_path and _matches(file_path, target_keys):
                patched_record = _patch_otrk_tcom(record_data, new_comment)
                out += tag
                out += struct.pack('>I', len(patched_record))
                out += patched_record
                pos += 8 + record_len
...
def _patch_otrk_tcom(otrk_data: bytes, new_comment: str) -> bytes:
    """
    Return a new otrk record body with the `tcom` field set to `new_comment`,
    encoded UTF-16BE with no BOM (matches Serato's own text-field encoding
    elsewhere in this codebase — see smart_crate.py's field writer).

    Unlike `utpc`, `tcom` is variable-length, so replacing it changes the
    record's total size — the caller (_patch_tcom) recomputes the enclosing
    otrk record's length from the returned bytes, same as it already does
    for every other field.

    If `tcom` already exists, it is replaced in place. If it doesn't exist,
    it is appended at the end of the record. All other fields are preserved
    unchanged.
    """
    out          = bytearray()
    pos          = 0
    dlen         = len(otrk_data)
    tcom_written = False
    new_value    = new_comment.encode('utf-16-be')
    ...
    while pos + 8 <= dlen:
        ...
        if tag_str == 'tcom':
            # Replace existing tcom with new value
            out += b'tcom'
            out += struct.pack('>I', len(new_value))
            out += new_value
            tcom_written = True
        else:
            out += otrk_data[pos: pos + 8 + field_len]
    ...
    if not tcom_written:
        # Insert tcom at the end of the record
        out += b'tcom'
        out += struct.pack('>I', len(new_value))
        out += new_value

...
class AddTracksCommand(Command):
    def __init__(
        ...
        reload_tracks: bool = True,
    ):
        self.view          = view
        self.crate_path    = crate_path
        self.track_paths   = list(track_paths)
        self.stay_on_crate = stay_on_crate  # if set, execute() keeps this crate selected
        # False when the visible track list can't have changed (added to a crate
        # other than the one on screen while All Tracks is selected) — skip the
        # table repopulate so its rows don't reshuffle.
        self.reload_tracks = reload_tracks
    ...
    def execute(self) -> None:
        ...
        self.view._refresh(
            select=self.stay_on_crate or self.crate_path,
            reload_tracks=self.reload_tracks,
        )

...
Right now `GenreClassifier` (`classifier.py`) is purely local and has zero knowledge of who any artist actually is. It classifies in strict tiers — exact tag match → style-map lookup → comment/style-token analysis → folder-name hint (deliberately skipped inside "purpose" folders like `_samples`/`_tributes`/`_instrumentals`, since those describe use-case, not genre) → Unclassified. If a file's own tag, folder, and comment data are all empty or unhelpful, the classifier has nothing left to reach for — no internet lookup, no artist database, no cross-referencing, regardless of how famous or well-documented the artist is.
